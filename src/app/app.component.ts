...
export class AppComponent implements OnDestroy {
    ...
    OPENVIDU_SERVER_URL = 'https://134.209.168.81:4443';
    ...
    async presentAlert(){
        const alert = await this.alertController.create({
            header:'UYARI',
            subHeader:'Yayın Kapatıldı!',
            message: this.myUserName+ ' kişisi yayını kapattı',
            buttons:['Tamam']
        });
        await alert.present();
    }
    joinSession() {
        ...
        this.getToken().then((token) => {
            ...
            this.session.on('signal:my-chat', (event: SignalEvent) => {
            console.log("GELDI"+event.data);
            this.messageList.push(JSON.parse(event.data).username+JSON.parse(event.data).message );
    ...
    sendMessage(): void {
            this.myMessage = this.myMessage.replace(/ +(?= )/g, '');
            if (this.myMessage !== '' && this.myMessage !== ' ') {
                const data = {
                    connectionId: this.mySessionId,
                    username: this.myUserName,
    ...
    leaveSession() {
        ...
        if(this.usertype=="pub"){
            
            this.session.onParticipantUnpublished(this.myUserName+" kişisi yayını kapattı");
        }
...
/*
    videoshow() {
        if(this.videok==false) {
            this.session.publish(this.publisher.publishVideo(true));
            this.videok=true;
        } else {
    ...
    audioshow() {
        if(this.audiok==false) {
            this.session.publish(this.publisher.publishAudio(true));
            this.audiok=true;
        } else {
        this.session.publish(this.publisher.publishAudio(false));
        this.audiok=false;
        }
